...
const CardPublications = ({ publication, equip, updateFav, remove, action, user }) => {
  ...
  return (
    <section className='flex flex-col justify-between h-32 overflow-hidden bg-gray-100 rounded-md shadow-sm w-60 shadow-aero'>
      <Link className='flex' href={equip ? `/publicaciones/juegos/${publication.id}?page=${COLLECTIONS.teams}` : `/publicaciones/juegos/${publication.id}?page=${publication.category}`}>
        ...
        <div>
          ...
          <p className='pl-1 text-sm'>{publication.description.slice(0, 30)}...</p>
        </div>
      </Link>
