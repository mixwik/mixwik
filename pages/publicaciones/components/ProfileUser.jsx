...
const ProfileUser = ({ mixWikTeams, currentUser, idPublication, user }) => {
  ...
  return (
    <div className={`sticky top-0 md:top-[6vh] bg-transparent z-10 flex justify-between w-full md:w-[40vw] text-white bg-blue-300 ${mixWikTeams && 'md:pt-[1.8rem]'}`} target='_blanc'>
      <Link className='flex items-center w-full gap-1 font-semibold bg-pennBlue' href={`/user/${currentUser.uid}`}>
      ...
      <button onClick={() => handleFavorites(visitorUser.id, idPublication)} className={`flex items-center bg-orange justify-center w-20 text-black ${like && 'bg-red-500'}`}>
        {
          like
            ? (
