...
export const EditImages = ({ category, id, publicationUser, prevImg, prevImg2, prevImg3, prevImg4, prevImg5, prevImg6, prevImg7, setEdit, mixWikTeams }) => {
  ...
  const handleSetImage = async (e, setImages, setImgsURL, setPreviewImages) => {
    ...
    if (
      ...
      e.target.files[0].name !== image7.name
        ...
        setImageDB(publicationUser.uid, e.target.files[0], setImgsURL, setProgress)
  ...
  const handleRemoveImage = async (e, images, setPreviewImages, setImages, setImgsURL) => {
    e.preventDefault()
    removeImageDB(publicationUser.uid, images.name || images)
    setPreviewImages('')
    setImages('')
    setImgsURL('')
    setProgress(true)
